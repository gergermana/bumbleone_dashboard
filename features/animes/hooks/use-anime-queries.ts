...
import { animeQueryOptions } from "../api/anime-queries";

export const useAnime = (filters?: any) => {
    return useQuery(animeQueryOptions.all(filters));
}

export const useAnimeDetail = (id: string) => {
    return useQuery(animeQueryOptions.detail(id));
}
